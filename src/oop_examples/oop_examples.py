...
class MyDataFrame(pd.DateFrame):
    def num_cells(self):
...
class Car:
    """An example of a class with attributes"""

    def __init__(self, fuel, maxspeed=120, speed=0):
      """
    ...
    def setSpeed(self, new_speed):
        if new_speed > self.maxspeed:
            return 'You cannot exceed the max speed'
        else:
...
class Animal:
    def __init__(self, name, weight, diet_type):
        """A genreal representation of animals"""
        self.name = str(name)
...
"""This only gets run when the file is run, not on import"""
...
if __name__ == "__main__":
    ...
    user2 = SocialMediaUser(
        name='Carlos', location='West Virginia', upvotes=200000010203)
    user3 = SocialMediaUser(name='Carlita', location='Northwest Virginia')
